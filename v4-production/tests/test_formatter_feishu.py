...
# ---------------------------------------------------------------------------
# Body elements: source link (markdown)
# ---------------------------------------------------------------------------


class TestSourceLink:
    def test_source_link_present_when_url_exists(self, full_article: dict) -> None:
        card = json_to_feishu(full_article)
        links = [e for e in card["body"]["elements"]
                 if e["tag"] == "markdown" and "查看原文" in e.get("content", "")]
        assert len(links) == 1

    def test_source_link_contains_url(self, full_article: dict) -> None:
        card = json_to_feishu(full_article)
        link = _find_source_link(card)
        assert "github.com/openai/gpt-5" in link["content"]

    def test_source_link_absent_when_no_url(self, minimal_article: dict) -> None:
        card = json_to_feishu(minimal_article)
        links = [e for e in card["body"]["elements"]
                 if e.get("tag") == "markdown" and "查看原文" in e.get("content", "")]
        assert len(links) == 0

    def test_source_link_before_hr(self, full_article: dict) -> None:
        card = json_to_feishu(full_article)
        elements = card["body"]["elements"]
        link_idx = next(i for i, e in enumerate(elements) if "查看原文" in e.get("content", ""))
        hr_idx = next(i for i, e in enumerate(elements) if e["tag"] == "hr")
        assert link_idx + 1 == hr_idx, "source link must immediately precede hr"


# ---------------------------------------------------------------------------
# Body elements: hr (footer divider)
# ---------------------------------------------------------------------------
...
class TestFooter:
    ...
    def test_hr_is_last_element(self, minimal_article: dict) -> None:
        card = json_to_feishu(minimal_article)
        elements = card["body"]["elements"]
...
class TestEdgeCases:
    def test_empty_dict_returns_valid_card(self, minimal_article: dict) -> None:
        ...
        assert len(card["body"]["elements"]) == 2  # div, hr

    def test_minimal_card_element_count(self) -> None:
        """No insight, no summary, no source_url, no tags → div + hr = 2."""
        card = json_to_feishu({})
        assert len(card["body"]["elements"]) == 2

    def test_maximal_card_element_count(self, full_article: dict) -> None:
        """Insight + summary + tags + source_url → insight + summary + div + link + hr = 5."""
        card = json_to_feishu(full_article)
...
def _find_source_link(card: dict) -> dict:
    """Return the source-link markdown element from the card body."""
    for element in card["body"]["elements"]:
        if element.get("tag") == "markdown" and "查看原文" in element.get("content", ""):
            return element
    raise AssertionError("No source link element found in card body")
