...
用 `Read` 读 `knowledge/articles/index.json`（含每篇文章的 `id` / `title` / `category` / `quality_score` / `tags` / `created_at`）。

**在内存里筛**今日的条目（`created_at` 字段以今日日期开头）；今日无数据则回退到最近 7 天。

> **不要尝试 Glob 或 grep 文件名** —— 索引文件已经聚合了所有元信息，一次 Read 就够了。

### Step 2: 内存过滤 + 排序

1. 过滤 `quality_score >= 0.6` 的条目
2. 按 `quality_score` 降序排序
3. 取 Top 5
